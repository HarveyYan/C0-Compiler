...
typedef enum{
    ldc, opr, lod, sto, cal, inc, jmp, jpc, red, wrt, init_p, init_main
} fct;
/*
 * ldc 0 a 加载整数常数
 * ldc 1 a 加载浮点常数
...
int int_num, cc, ll, cx, line_count, err, tx, _global_array_marker = 0, s[stack_max];
float float_num;
...
void print_pcode(){
    int i;
    char *fun[] = {"ldc", "opr", "lod", "sto", "cal", "inc", "jmp", "jpc", "red", "wrt", "init_p", "init_main"};
    for (i = 0; i < cx; i++){
        if (code[i].f==ldc && code[i].l == 1){
        ...
        if (tab[pos].obj == constant){
            ...
            if (tab[pos].typ == arrays){
                ...
                if (sym == lbrack){
                    getsym();
                    if (sym == intcon){
                        if (tab[pos].ref.arr_size <= int_num){
                            error(23);
                            int_num = 0; //改正错误
                        }
                        else{
                            tmp.inum = tab[pos].addr.inum + int_num;
    ...
    if (sym == ifsy){
        ...
        if (pos != 0){
            if (tab[pos].obj==variable){    //赋值语句
                if (tab[pos].typ == arrays){    //函数赋值
                    getsym();
                    if (sym == lbrack){
                        getsym();
                        if (sym == intcon){
                            dx = int_num;
                            if (dx >= tab[pos].ref.arr_size){//数组越界
                                error(23);
                                dx=0;   //改正错误
                            }else{
                                getsym();
...
int counter_t_inc(int pos){ //返回的是个数
    int i, dx;
    for (i=pos+1 ; i<tx ; i++){
    ...
    do{
        ...
        switch(ins.f){
            ...
            case init_main:
                ...
                s[t+5] = ins.a.inum;
                b = t+1;
                p = tab[s[b+4]].addr.inum;
                t = b + counter_t_inc(ins.a.inum) - 1;
                break;
            case cal:   //cal l a : l是层次差， a是函数在符号表登记位置
                i = b+counter_t_inc(s[b+4])-1;
                //s[b+4]是当前函数pos内容; i+1,i+2...t 是操作数栈的内容
                //i指向最后一个变量

                for (j = t-1; j >= i; j-- ){
                    s[j+counter_t_inc(ins.a.inum)+1] = s[j+1];  //新的操作数栈，这种移法不会破坏原栈的内容
                }
//                for (j = i; j < t; j++){
...
//                }
                ...
                s[i+1] = 0;                 //返回值
                s[i+2] = p;                 //返回地址
                s[i+3] = base(ins.l, b);    //静态链，这个地方应该都是0...
                s[i+4] = b;                 //动态链，返回的时候有用
                s[i+5] = ins.a.inum;        //符号表中函数位置
                b = i + 1;                  //基地址更新
                p = tab[s[b+4]].addr.inum;     //指令指向函数开始位置

                //接下来要写入参数，并将栈顶的临时实参清掉
                if (tab[s[b+4]].ref.params !=0){
                    for(i = tab[s[b+4]].ref.params; i > 0; i-- ){
                        s[b+4+i]=s[t];
                        t--;
                    }
            ...
            case jpc:
                ...
                break;
            case red:   //TODO 忽略浮点数
                scanf("%d",&s[base(ins.l, b) + ins.a.inum]);
                break;
            case wrt:   //TODO 忽略浮点数
                if (ins.l == 1){
                    printf("%s\n",ins.a.str);
