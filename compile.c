...
#define   ident_max     100      //标识符长度上限
#define   code_max      1000     //目标代码数组长度
#define   string_max    100      //字符串长度上限
#define   stack_max     5000     //运行栈长度上限
...
char ch=' ', line[200], file[50], err_msg[60][50], str[string_max];    //当前读到字符
                    ...
                    if (sym == intcon){
                        ...
                        if (type == floatsy){//整数转换成浮点数
                            ...
                            if (int_num=='+'||int_num=='-'||int_num ==
                                ...
                                //printf("%d: Implicit transformation\n", line_count);//隐式转换
                            }else{
                                int_num = 0;
                                error(11);
                            }
                        ...
                        printf("%s, %d",id, sym);
                        error(7);
                        symset[0] = comma, symset[1] = semicolon;
                        skip(2);
                    }
                    getsym();
                }
...
void factor(int level){
    ...
    if (sym == ident){//常量、变量 （含数组）或函数调用
        ...
        if (count > 2){ //三到多个符号，报语法错误
            error(9);
        }

        if (count == 2){
            sym = floatcon;
            tmp.fnum = (float)int_num;
            gen(ldc,2,tmp);
            getsym();
        }
        else{
    ...
    if (sym == ifsy){
        ...
        if (sym == lparent){
            ...
            if (sym == rparent){
                ...
                error(18);
                symset[0]=rparent,symset[1]=lbrace;
                skip(2);
                getsym();
            }
        }
...
void functiondeclaration(symbol returntype, int level){
    ...
    if (sym == lparent){
        ...
        if (sym == rparent){
            ...
            error(18);
            symset[0] = lbrace, symset[1] = rparent;
            skip(2);
            getsym();
        }
    }else{
...
void program(int level){    //从最外层开始
    ...
    if (sym != constsy && sym != intsy && sym != floatsy && sym != charsy && sym != voidsy){    //program接受的标识符
        error(50);
        symset[0]=constsy, symset[1]=intsy, symset[2]=floatsy,symset[3]=charsy,symset[4]=voidsy;
        skip(5);
    }
...
//                }
                ...
                s[i+1].type = tab[ins.a.inum].ref.return_typ, s[i+1].inum = 0, s[i+1].fnum = 0;                 //返回值
                    ...
                    for(i = tab[s[b+4].inum].ref.params; i > 0; i-- ){
                        ...
                        if (s[b+4+i].type == s[t].type){
                            ...
                            if (s[b+4+i].type == chars){    //TODO: double check
                                ...
                                if (s[t].type == floats){
                                    s[b+4+i].fnum = s[t].fnum;
                                }else{
                                    s[b+4+i].fnum = (float)s[t].inum;
                                }
                            }
                        }
            ...
            case lod:
                ...
                break;
            case lda:   //加载数组元素指令，需要用到栈顶(s[t])中计算出的下标值
                if (s[t].type == floats){
                    s[t].type = ints;
...
//                        }
                    ...
                    case 1:
                        s[t].inum *= -1;
                        s[t].fnum *= -1;
                        if (s[t].type==chars)   //改正: 2016.12.16
                            s[t].type=ints;
                        break;
                    case 2:
                        t = t-1;
...
int main(){
    ...
    strcpy(err_msg[17],"Expected a ]"); //rbracket
    strcpy(err_msg[18],"Expected a )"); //rparent
    strcpy(err_msg[19],"Expected a }"); //rbrace
    ...
    strcpy(err_msg[31],"Expected a ["); //lbracket
    strcpy(err_msg[32],"Expected a ("); //lparent
    strcpy(err_msg[33],"Expected a {"); //lbrace
    strcpy(err_msg[34],"Expected an operator"); //+,-,*,/
    strcpy(err_msg[35],"Variable type invalid");
